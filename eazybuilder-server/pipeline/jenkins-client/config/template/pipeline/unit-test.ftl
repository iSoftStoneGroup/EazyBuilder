                            ...
                            steps {
                                <#if project.projectType?? && project.projectType=="net">
                                    ...
                                    <#if project.netType?? && project.netType=="net5">
                                        sh """dotnet /root/.dotnet/tools/.store/dotnet-sonarscanner/5.5.3/dotnet-sonarscanner/5.5.3/tools/net5.0/any/SonarScanner.MSBuild.dll begin -k:"net-test" -d:sonar.host.url=${sonarUrl} -d:sonar.login=${sonarUser} -d:sonar.password=${sonarPassword} -d:sonar.cs.opencover.reportsPaths=${r"${env.COVERAGEPATH}"} -d:sonar.branch.name=${tagName!'master'}"""
                                    ...
                                    <#elseif project.netType?? && project.netType=="netcoreapp2">
                                        sh """dotnet /root/.dotnet/tools/.store/dotnet-sonarscanner/5.5.3/dotnet-sonarscanner/5.5.3/tools/netcoreapp2.0/any/SonarScanner.MSBuild.dll begin -k:"net-test" -d:sonar.host.url=${sonarUrl} -d:sonar.login=${sonarUser} -d:sonar.password=${sonarPassword} -d:sonar.cs.opencover.reportsPaths=${r"${env.COVERAGEPATH}"} -d:sonar.branch.name=${tagName!'master'}"""
                                        sh '''nuget restore <#if project.netSlnPath !="">${project.netSlnPath}<#else>${project.name}.sln</#if> && dotnet build <#if project.netSlnPath !="">${project.netSlnPath}<#else>${project.name}.sln</#if>'''
                                    ...
                                    <#elseif project.netType?? && project.netType=="netcoreapp3">
                                        sh """dotnet /root/.dotnet/tools/.store/dotnet-sonarscanner/5.5.3/dotnet-sonarscanner/5.5.3/tools/netcoreapp3.0/any/SonarScanner.MSBuild.dll begin -k:"net-test" -d:sonar.host.url=${sonarUrl} -d:sonar.login=${sonarUser} -d:sonar.password=${sonarPassword} -d:sonar.cs.opencover.reportsPaths=${r"${env.COVERAGEPATH}"} -d:sonar.branch.name=${tagName!'master'}"""
                                        sh '''nuget restore <#if project.netSlnPath !="">${project.netSlnPath}<#else>${project.name}.sln</#if> && dotnet build <#if project.netSlnPath !="">${project.netSlnPath}<#else>${project.name}.sln</#if>'''
