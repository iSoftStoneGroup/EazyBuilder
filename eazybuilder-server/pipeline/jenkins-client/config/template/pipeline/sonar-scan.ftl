                    ...
                    stage('sonar scan'){
                       steps {
                            ...
                            <#elseif project.projectType?? && project.projectType=="dataBaseScript">
                                sh '''mvn <#if !project.legacyProject && project.pomPath?? && project.pomPath !="">-f ${project.pomPath} </#if>sonar:sonar -Dmirror.url=${mirrorUrl}  -Dsonar.branch.name=${tagName!'master'} -Dmaven.repo.local=/usr/share/maven-repo/teams/${project.team.id} -Dsonar.scm.disabled=true -Dsonar.host.url=${sonarUrl} -Dsonar.login=${sonarUser} -Dsonar.password=${sonarPassword} -Dsonar.projectKey=${project.name}:${project.id} <#if project.profile?? && project.profile.skipJsScan?? && project.profile.skipJsScan> -Dsonar.exclusions=**/*.js  <#if project.profile.sonarProfile?? && project.profile.sonarProfile !="">-Dsonar.profile=${project.profile.sonarProfile} </#if> </#if> -Dsonar.language=sql -Dsonar.sql.dialect=${project.sqlType} -Dsonar.sources=<#if project.sqlPath !="">${project.sqlPath}<#else>config</#if>'''
                            <#else>
                                echo '========sonar scan start========'
                                sh '''if [ ! -d "node_modules" ]; then
                                mvn <#if !project.legacyProject && project.pomPath?? && project.pomPath !="">-f ${project.pomPath} </#if>sonar:sonar -Dmirror.url=${mirrorUrl}  -Dsonar.branch.name=${tagName!'master'} -Dmaven.repo.local=/usr/share/maven-repo/teams/${project.team.id} -Dsonar.scm.disabled=true -Dsonar.host.url=${sonarUrl} -Dsonar.login=${sonarUser} -Dsonar.password=${sonarPassword} -Dsonar.projectKey=${project.name}:${project.id} <#if project.profile?? && project.profile.skipJsScan?? && project.profile.skipJsScan> -Dsonar.exclusions=**/*.js  <#if project.profile.sonarProfile?? && project.profile.sonarProfile !="">-Dsonar.profile=${project.profile.sonarProfile} </#if> </#if>
                                else
                                mvn <#if !project.legacyProject && project.pomPath?? && project.pomPath !="">-f ${project.pomPath} </#if>sonar:sonar -Dmirror.url=${mirrorUrl} -Dsonar.branch.name=${tagName!'master'} -Dmaven.repo.local=/usr/share/maven-repo/teams/${project.team.id} -Dsonar.scm.disabled=true -Dsonar.host.url=${sonarUrl} -Dsonar.login=${sonarUser} -Dsonar.password=${sonarPassword} -Dsonar.projectKey=${project.name}:${project.id} <#if project.profile?? && project.profile.skipJsScan?? && project.profile.skipJsScan>-Dsonar.exclusions=**/*.js</#if> <#if project.profile.staticJs?? && project.profile.staticJs> -Dsonar.modules=javascript-module  -Djavascript-module.sonar.projectBaseDir=./ -Djavascript-module.sonar.sources=src </#if>  <#if project.profile.sonarProfile?? && project.profile.sonarProfile !="">-Dsonar.profile=${project.profile.sonarProfile} </#if>
                                fi'''
                                echo '========sonar scan end========'
                            </#if>
