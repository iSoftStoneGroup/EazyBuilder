<#if !project.profile?? || !project.profile.skipDependencyCheck>
            stage('dependency check'){
               steps {
                  ...
                  <#else>
                    sh '''mvn <#if !project.legacyProject && project.pomPath?? && project.pomPath !="">-f ${project.pomPath} </#if>org.owasp:dependency-check-maven:6.0.2:aggregate -DretireJsUrl=http://0.0.0.0:8080/mirror/retire.js/raw/master/repository/jsrepository.json -Dmaven.repo.local=/usr/share/maven-repo/teams/${project.team.id} -Dformat=ALL -DautoUpdate=false -DskipProvidedScope=true -DskipSystemScope=true -Dmirror.url=${mirrorUrl}'''
                  </#if>
