<#if !project.profile?? || !project.profile.skipUnitTest>
                       // stage('run unit test') {
                      //      steps {
                       //      sh '''mvn <#if !project.legacyProject && project.pomPath?? && project.pomPath !="">-f ${project.pomPath} </#if>test --fail-never -Dmaven.repo.local=/usr/share/maven-repo/teams/${project.team.id} -Dmaven.test.failure.ignore=true -Dsurefire.timeout=300 -Dmirror.url=${mirrorUrl} '''
                       //     }
                       // }
