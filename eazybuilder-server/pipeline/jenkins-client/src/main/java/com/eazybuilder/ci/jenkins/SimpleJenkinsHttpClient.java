...
public class SimpleJenkinsHttpClient extends JenkinsHttpClient{
     ...
     */
    public SimpleJenkinsHttpClient(URI uri, CloseableHttpClient client) {
    	super(uri,client);
     ...
     */
    public SimpleJenkinsHttpClient(URI uri, HttpClientBuilder builder) {
     ...
     * @param uri Location of the jenkins server (ex. http://localhost:8080)
     */
    public SimpleJenkinsHttpClient(URI uri) {
     ...
     */
    public SimpleJenkinsHttpClient(URI uri, String username, String password) {
    ...
    public <T extends BaseModel> T get(String path, Class<T> cls) throws IOException {
        ...
        LOGGER.debug("查询jenkins api获取流水线执行状态:{}",getMethod);
    ...
    public <T> T getDirectly(String path, Class<T> cls) throws IOException {
        HttpGet getMethod = new HttpGet(noapi(path));
        getMethod.setConfig(requestConfig);
        HttpResponse response = client.execute(getMethod, localContext);
        getJenkinsVersionFromHeader(response);
        try {
            ...
            return getDefaultMapper().readValue(bytes, cls);
        } finally {
            EntityUtils.consume(response.getEntity());
            releaseConnection(getMethod);
        }
    ...
    public InputStream getStreamDirectly(String path) throws IOException {
    	 HttpGet getMethod = new HttpGet(noapi(path));
         getMethod.setConfig(requestConfig);
         HttpResponse response = client.execute(getMethod, localContext);
         getJenkinsVersionFromHeader(response);
    ...
    public <R extends BaseModel, D> R post(String path, D data, Class<R> cls, boolean crumbFlag) throws IOException {
        ...
        if (crumbFlag == true) {
            ...
            if (crumb != null) {
                ...
                request.addHeader(new BasicHeader(HttpHeaders.COOKIE,this.cookie));
                request.addHeader(new BasicHeader("X-Jenkins-Session", this.jenkinsSession));
            }
        }

        if (data != null) {
            String value = getDefaultMapper().writeValueAsString(data);
     ...
     * @throws IOException in case of an error.
     */
    public void post_formData(String path, Map<String, Object> data, boolean crumbFlag) throws IOException {
        HttpPost request;
        if (data != null) {
    ...
    private <T extends BaseModel> T objectFromResponse(Class<T> cls, HttpResponse response) throws IOException {
        InputStream content = response.getEntity().getContent();
        byte[] bytes = ByteStreams.toByteArray(content);
        T result = getDefaultMapper().readValue(bytes, cls);
        // TODO: original:
    ...
    private void getJenkinsVersionFromHeader(HttpResponse response) {
        ...
        try {
        	...
        	if(headers.length>0) {
        		this.cookie=headers[0].getValue();
        	}
        	headers=response.getHeaders("X-Jenkins-Session");
        	if(headers.length>0) {
        		this.jenkinsSession=headers[0].getValue();
        	}
		} catch (Exception e) {
		}
