app.controller('hostInfoController', function($scope,$http,$window,$state,$filter,$modal,basicService) {
	$scope.condition={};
	$scope.entity={};
	...
	$scope.tableControl={
		options:{
            url:backend.url+"/api/hostInfo/page",
        	cache:false,
            idField:'id',
	...
	$scope.add=function(){
		...
		$state.go('hostInfo.add');
	}
	
	
	$scope.save=function(){
		$http.post(backend.url+"/api/hostInfo",$scope.entity).then(function(response){
			alert("保存成功");
			$state.go("hostInfo.list");
		});
	}
	
	$scope.viewDetail = function (row) {
		$scope.entity=angular.copy(row);
		$state.go('hostInfo.edit');
	}
