app.controller('itPlanController', function($scope,$http,$window,$state,$filter,$modal,basicService) {
	$scope.condition={};
	$scope.entity={};
	...
	$scope.tableControl={
		options:{
            url:backend.url+"/api/itPlan/page",
        	cache:false,
            idField:'id',
	...
	$scope.showAddModel=function(env){
		...
		var modalInstance=$modal({
			  title: '选择测试脚本', 
			  templateUrl: 'app/itPlan/scriptModal.html', 
		      show: true,
		      ...
		      controller:function($scope){
		    	  ...
		    	  $scope.scriptTableControl={
		    				options:{
		    		            url:backend.url+"/api/atmScript/pageWithExcludes",
		    		        	cache:false,
		    		            idField:'id',
		    		            queryParams:function(params){
		    		            	var queryParam=angular.extend({},params,$scope.condition);
	...
	$scope.save=function(){
		...
		$http.post(backend.url+"/api/itPlan",$scope.entity).then(function(response){
			alert("保存成功");
			$state.go("itPlan.list");
		});
	}
	...
	$scope.execute=function(){
		...
		if(confirm("确认立即执行测试计划["+selected[0].name+"]?")){
			$http.get(backend.url+"/it/run?id="+selected[0].id+"&uid="+guid()).then(function(){
