app.controller('appscanScriptController', function($scope,$http,$window,$state,$filter,basicService) {
	$scope.condition={};
	$scope.entity={};
	...
	$scope.tableControl={
		options:{
            url:backend.url+"/api/appscan/script/page",
        	cache:false,
            idField:'id',
	...
	$scope.add=function(){
		$scope.entity={scm:{type:'svn'}};
		$state.go('appScanScript.add');
	}
	...
	$scope.save=function(){
		...
		$http.post(backend.url+"/api/appscan/script",$scope.entity).then(function(response){
			alert("保存成功");
			$state.go("appScanScript.list");
		});
	}
	...
	$scope.remove= function(){
		...
		if(confirm("确认删除选中项？")){
			$http({
				url:backend.url+"/api/appscan/script",
				method:"delete",
				headers: {
