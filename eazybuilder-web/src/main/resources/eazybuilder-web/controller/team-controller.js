app.controller('teamController', function($scope,$rootScope,$http,$window,$state,$filter,basicService) {
	...
	$scope.save=function(){
		$scope.entity.qualityprofilesJson = JSON.stringify($scope.entity.teamQualityProfilesMap);
		...
		$http.post(backend.url + "/api/sonarqube/qualityprofilesAddProject?teamId="+$scope.entity.id,JSON.stringify($scope.entity.teamQualityProfilesMap));
	...
	$scope.viewDetail = function (row) {
		...
		$scope.entity.teamQualityProfilesMap = JSON.parse($scope.entity.qualityprofilesJson);
		// $http.get(backend.url + "/api/project/projectsByTeamId/" + $scope.entity.id).then(function (response) {
		// 	$scope.projects = response.data;
		// });
		$http.get(backend.url + "/api/sonarqube/getLanguagesList/").then(function (response) {
