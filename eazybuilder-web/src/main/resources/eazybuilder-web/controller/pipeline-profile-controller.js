	...
	$scope.viewDetail = function (row) {
		var modalInstance=$modal({
			...
			controller:function($scope,$sce){
				...
				$scope.getCheckedTeam = function(){
					if($scope.teams && +$scope.cracker.teamId){
						$scope.cracker.team = $scope.teams.filter(team=>team.id == $scope.cracker.teamId)[0];
					}
				}
				$scope.switch=function(){
					$modal({
						templateUrl: 'app/pipelineProfile/switchImage.html',
					});
				}
				$scope.crackerSkipCloneCodeOff = function () {
					$scope.cracker.skipCloneCode=true;
					$scope.cracker.skipMvnBuild=true;
					$scope.cracker.addTag=false;
					$scope.cracker.updateTag=false;
					$scope.cracker.createBranch=false;
					$scope.cracker.skipScan=true;
					$scope.cracker.skipUnitTest=true;
					$scope.cracker.assignYaml=false;
					$scope.cracker.sqlScript=false;
					$scope.cracker.updateConfig=false;
					$scope.cracker.skipDockerBuild=true;
				}
				$scope.crackerSkipMvnBuildOn = function () {
					$scope.cracker.skipMvnBuild=false;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerSkipMvnBuildOff = function () {
					$scope.cracker.skipMvnBuild=true;
					$scope.cracker.skipDockerBuild=true;
				}
				$scope.crackerAddTagOn = function () {
					$scope.cracker.addTag=true;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerUpdateTagOn = function () {
					$scope.cracker.updateTag=true;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerCreateBranchOn = function () {
					$scope.cracker.createBranch=true;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerSkipScanOn = function () {
					$scope.cracker.skipScan=false;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerSkipUnitTestOn = function () {
					$scope.cracker.skipUnitTest=false;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerAssignYamlOn = function () {
					$scope.cracker.assignYaml=true;
					$scope.cracker.skipCloneCode=false;
					$scope.cracker.skipDeploy=true;
					$scope.cracker.restartDeploy=false
				}
				$scope.crackerSqlScriptOn = function () {
					$scope.cracker.sqlScript=true;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerUpdateConfigOn = function () {
					$scope.cracker.updateConfig=true;
					$scope.cracker.skipCloneCode=false;
				}
				$scope.crackerSkipDockerBuildOn = function () {
					$scope.cracker.skipDockerBuild=false;
					$scope.crackerSkipMvnBuildOn();
				}
