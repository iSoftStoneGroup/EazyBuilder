app.controller('developerController', function($scope,$http,$window,$state,$filter,basicService) {
	$scope.condition={};
	$scope.entity={};
	$scope.tableControl={
		options:{
            url:backend.url+"/api/developer/page",
        	cache:false,
            idField:'id',
	...
	$scope.add=function(){
		$scope.entity={};
		$state.go('developer.add');
	}
	
	$scope.save=function(){
		$http.post(backend.url+"/api/developer",$scope.entity).then(function(response){
			alert("保存成功");
