app.controller('projectController', function($scope,$http,$window,$state,$filter,$modal,basicService) {
	...
	$scope.addQueryParam=function(data){
		...
		data.push({deployConfigDetailEnvs:[]},{deployConfigDetailHosts:[]});
	}
	...
	$scope.viewDeployConfigDetail = function (index,deployConfig) {
		...
		var modalInstance = $modal({
				...
				controller: function ($scope) {
					...
					$scope.addModelQueryParamHosts=function(data){
						if(!data){
							data=[];
						}
						data.push({});
					}
					$scope.delModelQueryParamHosts=function(index,data){
						data.splice(index, 1);
					}
	...
	$scope.createDeployConfig1 = function(){
		var name = $scope.entity.name;
		var regTeamCode = /^[A-Za-z]+[A-Za-z-]*$/;
		if (!(regTeamCode.test(name))){
			alert("项目英文名称: 必须以字母开头，只能包含 字母 或 中划线- ");
		}
	}
	$scope.sourceWrehouseAddress1 = function(){
		var scmUrl = $scope.entity.scm.url;
